package com.newsreels.app.storyMaker;
...
import com.newsreels.app.utills.ResizeWidthAnimation;

final class HorizontalPausableProgressBar extends FrameLayout {
    ...
    public HorizontalPausableProgressBar(Context context) {
        this(context, null);
    }

    public HorizontalPausableProgressBar(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HorizontalPausableProgressBar(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        LayoutInflater.from(context).inflate(R.layout.horizontal_pausable_progress, this);
        bullet = findViewById(R.id.bullet);
        back_progress = findViewById(R.id.back_progress);
...
//        frontProgressView.setBackground(getContext().getResources().getDrawable(R.drawable.bullet_selector));
    ...
    public void setWidth(int width, boolean animate) {
        ResizeWidthAnimation anim = new ResizeWidthAnimation(bullet, width);
        if (animate) {
    ...
    public void setWidthWithCallback(int width, boolean animate, Animation.AnimationListener animationListener) {
        ResizeWidthAnimation anim = new ResizeWidthAnimation(bullet, width);
        if (animate) {
            anim.setDuration(Constants.mProgressAnimationSpeed);
        } else {
...
//            maxProgressView.setBackgroundResource(R.color.progress_secondary);
    ...
    public void reverseProgress(Animation.AnimationListener animationListener) {
        setWidthWithCallback(Constants.mProgressCollapseSize, true, animationListener);
    }

    public void reverseProgressPrev(Animation.AnimationListener animationListener) {
        maxProgressView.setBackgroundResource(R.drawable.bullet_selector_2);
        setWidthWithCallback(Constants.mProgressCollapseSize, true, animationListener);
    }
