...
public class InstagramTabLayout extends FrameLayout {
    ...
    public InstagramTabLayout(Context context, List<Page> items, PictureSelectionConfig config) {
        ...
        if (config.instagramSelectionConfig.getCurrentTheme() == THEME_STYLE_DARK) {
            selectedIndicatorPaint.setColor(ContextCompat.getColor(context, R.color.white));
        }  else {
            selectedIndicatorPaint.setColor(ContextCompat.getColor(context, R.color.black));
        }
    ...
    public void selectTab(int position) {
        ...
        for (int i = 0; i < tabViews.size(); i++) {
            ...
            if (position == i) {
                if (config.instagramSelectionConfig.getCurrentTheme() == THEME_STYLE_DARK) {
                    ((TextView) tabView).setTextColor(ContextCompat.getColor(getContext(), R.color.white));
                }  else {
                    ((TextView) tabView).setTextColor(ContextCompat.getColor(getContext(), R.color.black));
                }
            } else {
                if (config.instagramSelectionConfig.getCurrentTheme() == THEME_STYLE_DARK) {
