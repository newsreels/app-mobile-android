...
class RangeSeekBarView @JvmOverloads constructor(
    ...
    private fun init() {
        ...
        val shadowColor = ContextCompat.getColor(context, R.color.shadow)
        mShadow.isAntiAlias = true
        mShadow.color = shadowColor
        mShadow.alpha = 177

        val lineColor = ContextCompat.getColor(context, R.color.menu_line)
...
//        drawShadow(canvas)
    ...
    private fun calculateThumbValue(index: Int) {
        if (index < thumbs.size && thumbs.isNotEmpty()) {
            ...
            val right: Float
            if (index == 0) {
                left = (th.pos + 1.75 * th.widthBitmap).toFloat()
                right = 0f
            } else {
                left = 0f
                right = th.pos + th.widthBitmap
            }

            onSeek(this, index, th.value, left, right)
        }
    }
    ...
    private fun onSeek(
        ...
        if (mListeners == null) return
            ...
            for (item in mListeners!!) {
                item.onSeek(rangeSeekBarView, index, value, left, right)
            }
        }
    }
